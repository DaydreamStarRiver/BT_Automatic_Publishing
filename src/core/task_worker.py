...
class TaskWorker:
    ...
    def start(self):
        ...
        logger.info("=" * 60)
        logger.info("🚀 Worker 启动 - 开始处理任务队列")
        logger.info("=" * 60)
    ...
    def _process_task(self, task):
        ...
        if task.status == TaskStatus.TORRENT_CREATED or (success and current_status == TaskStatus.NEW):
            self._handle_upload(task)
    
    def _handle_new_task(self, task):
        task_id = task.id
        
        logger.info(f"[{task_id}] 状态转换: NEW → TORRENT_CREATING")
    ...
    def _handle_upload(self, task):
        ...
        logger.info(f"[{task_id}] 状态转换: TORRENT_CREATED → UPLOADING")
        ...
        if result.get("success") or result.get("mode") == "preview":
            ...
            if task.can_retry():
                ...
                task.update_status(TaskStatus.PERMANENT_FAILED, f"超过最大重试次数 ({task.max_retries})")
                self.task_queue.persistence.save_task(task)
